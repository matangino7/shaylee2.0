...
def trigger_broadcast():
    ...
    try:
        ...
        if success:
            return jsonify({"status": "success", "message": "Broadcast triggered successfully!"})
        else:
            return jsonify({"status": "error", "message": "Error triggering broadcast. Please try again."}), 500
    except Exception as e:
        return jsonify({"status": "error", "message": f"Error triggering broadcast: {e}"}), 500
...
def main() -> None:
    ...
    dispatcher.add_handler(CommandHandler("sendates", functions_matan.send_dates))
    dispatcher.add_handler(MessageHandler(Filters.text & ~Filters.command, functions_matan.collect_dates))
